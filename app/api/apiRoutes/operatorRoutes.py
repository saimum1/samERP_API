...
@operatorController.post("/", response_model=operatorSchema.OperatorOut)
async def create_operator(operator: operatorSchema.OperatorCreate, db = Depends(get_db)):
    if operator.status and operator.status not in ["available", "not_available"]:
        raise HTTPException(status_code=422, detail="Status must be 'available' or 'not_available'")
    return await operatorMethod.create_operator(db, operator)

@operatorController.get("/", response_model=list[operatorSchema.OperatorOut])
async def list_operators(db = Depends(get_db)):
    return await operatorMethod.get_operators(db)

@operatorController.get("/{operator_id}", response_model=operatorSchema.OperatorOut)
async def get_operator(operator_id: str, db = Depends(get_db)):
    return await operatorMethod.get_operator_by_id(db, operator_id)

@operatorController.put("/{operator_id}", response_model=operatorSchema.OperatorOut)
async def update_operator(operator_id: str, operator: operatorSchema.OperatorUpdate, db = Depends(get_db)):
    if operator.status and operator.status not in ["available", "not_available"]:
        raise HTTPException(status_code=422, detail="Status must be 'available' or 'not_available'")
    return await operatorMethod.update_operator(db, operator_id, operator)

@operatorController.delete("/{operator_id}")
async def delete_operator(operator_id: str, db = Depends(get_db)):
    return await operatorMethod.delete_operator_by_id(db, operator_id)
